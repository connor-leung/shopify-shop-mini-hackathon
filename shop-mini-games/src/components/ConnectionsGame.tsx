...
export default function ConnectionsGame({
  ...
  useEffect(() => {
    if (gameOver) {
      ...
      onFinish({
        won,
        solvedCategories: categories.map((c) => ({
  ...
  if (error) {
    return (
      <div className="text-red-600 p-4 text-center">Error: {error.message}</div>
    );
  }
  ...
  const renderItem = (item: (typeof allItems)[0]) => {
    ...
    const baseClasses =
      "border rounded p-1 cursor-pointer select-none transition-colors flex items-center justify-center aspect-square overflow-hidden w-full";
    const statusClasses =
      status === "selected"
    ...
    return (
      ...
      >
        {(() => {
          const imgUrl =
            item.product?.featuredImage?.url || item.product?.images?.[0]?.url;
          if (imgUrl) {
            return (
              <img
                src={imgUrl}
          ...
          return (
            <span className="text-sm text-gray-700 w-full text-center px-1">
      ...
      <div className="mt-6 text-xs text-gray-500 space-y-1">
        {categories.map((cat) => (
          <div key={cat.difficulty}>
            <span className="font-semibold">{cat.difficulty}:</span>{" "}
            {cat.category} –{" "}
            {cat.items
              .map((it) => `${it.product?.title} (${it.product?.id})`)
              .join(", ")}
          </div>
        ))}
      </div>
